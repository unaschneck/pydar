...
field_options = [
    "act_pol_angle", "act_incidence_angle", "act_azimuth_angle", "burst_id",
    "sar_azimuth_res", "sar_range_res", "sigma0_corrected",
    "sigma0_uncorrected", "sigma0_uncorrected_std"
]
...
def sbdrMakeShapeFile(filename=None,
    ...
    def azimuth(lat1, lat2, long1, long2, radius):
        ...
        long1, lat1, long2, lat2 = map(math.radians,
                                       [long1, lat1, long2, lat2])

        diff_lats = lat2 - lat1
        diff_long = long2 - long1
        a = math.sin(
            diff_lats / 2)**2 + math.cos(lat1) * math.cos(lat2) * math.sin(
                diff_long / 2)**2
    ...
    gfields = {}
    for field in fields:
        if field.upper() in list(
                info):  # checks that field is an element in the info dataframe
    ...
    for i in range(
            num_record):  # TODO: check if i should be the index or the value?
        ...
        ptx = [pt1x, pt2x, pt3x,
               pt4x]  # TODO: duplicate of below line, remove?
        ...
        if (min([abs(i) - 360
                 for i in pty]) < 2) or (min([abs(i) for i in pty]) < 2):
