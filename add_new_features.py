...
if __name__ == "__main__":
    ...
    os.system(
        "git diff pydar/data/feature_name_details.csv | grep '^[+-][^+-]'")
    ...
    if not features_in_csv == retrieved_features:
        ...
        all_missing_features = [
            x for x in features_in_csv if x not in retrieved_features
        ]
        for feature_missing in all_missing_features:
            print(feature_missing)
            feature_row = features_df[features_df["Feature Name"] ==
                                      feature_missing]
            missing_lat_long = feature_row.columns[
                feature_row.isna().any()].tolist()
    ...
    print(
        "Update 'feature_name_full_list' in pydar/pytests/test_error_retrieve_ids_by_time_position.py"
    )
