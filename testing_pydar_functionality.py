...
if __name__ == '__main__':
	...
	print("\nFlyby IDS based on Latitude/Longitude = {0}".format(flyby_ids_with_segments))

	flyby_ids_range = pydar.retrieveIDSByLatitudeLongitudeRange(min_latitude=-82,
																max_latitude=-72,
																min_longitude=183,
																max_longitude=185)
	print("\nFlyby IDS based on Latitude/Longitude Range = {0}".format(flyby_ids_range))

	feature_names_list = pydar.retrieveFeaturesFromLatitudeLongitude(latitude=-72, longitude=183)
	print("\nFeature Names Found at -72 latitude and 183 longitude = {0}".format(feature_names_list))

	feature_names_list = pydar.retrieveFeaturesFromLatitudeLongitudeRange(min_latitude=-82,
																		max_latitude=-72,
																		min_longitude=183,
																		max_longitude=190)
	print("\nFeature Names Found in Latitude/Longitude Range = {0}".format(feature_names_list))

	flyby_ids_time = pydar.retrieveIDSByTime(year=2005, doy=301)
	print("\nFlyby IDS based on a specific timestamp = {0}".format(flyby_ids_time))
	...
	flyby_ids_time_range = pydar.retrieveIDSByTimeRange(start_year=2004,
														...
														end_second=23,
														end_millisecond=987)
	print("\nFlyby IDS based on a range of timestamps = {0}".format(flyby_ids_time_range))

	# Convert Flby Id into an Observation Number
	flyby_id_value = "T65"
	observation_num = pydar.convertFlybyIDToObservationNumber(flyby_id=flyby_id_value)
	print("\nFlyby ID '{0}' is observation number = {1}".format(flyby_id_value, observation_num))
	...
	#pydar.displayImages(image_directory="pydar_results/CORADR_0211_V03_S01/", figsize_n=5)
